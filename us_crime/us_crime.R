...
mcmc_res2 <- matrix(NA, 60, 10)
renorm_res2 <- matrix(NA, 60, 10)
lastresult <- 0
crime_mjmcmc_files <- list.files(path=paste0("data/mjmcmc/crime/test/"))
for (file in crime_mjmcmc_files[1:10]) {
  # Load the file
  cat("Loading ",file,"...\n")
  rundata <- loadRdata(paste0("data/mjmcmc/crime/test/",file))
  lastresult <- lastresult + 1

  cat("\nCalculating MCMC estimates...\n")
  mcmc_res2[,lastresult] <- rmse_conv(full_renorm, rundata, 60, F)
  cat("Calculating renormalized estimates...\n")
  renorm_res2[,lastresult] <- rmse_conv(full_renorm, rundata, 60, T,T)
}
