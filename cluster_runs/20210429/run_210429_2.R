...
full_100Kg <- mclapply(1:32, function (x) {
  run_sim(million_x, million_y_g, gaussian.loglik.aic, 100000, model_partitions[x,], 1)
}, mc.cores = num_cores)
full_100Kg <- unlist(full_100Kg, recursive = F)
save(full_100Kg, file="full_100K_g.Rdata")

full_100Kg_025 <- mclapply(1:32, function (x) {
  run_sim(million_x, million_y_g, gaussian.loglik.aic.irlssgd, 100000, model_partitions[x,], 0.0025)
}, mc.cores = num_cores)
...
full_100Kg_05 <- mclapply(1:32, function (x) {
  run_sim(million_x, million_y_g, gaussian.loglik.aic.irlssgd, 100000, model_partitions[x,], 0.005)
}, mc.cores = num_cores)
...
full_100Kg_075 <- mclapply(1:32, function (x) {
  run_sim(million_x, million_y_g, gaussian.loglik.aic.irlssgd, 100000, model_partitions[x,], 0.0075)
}, mc.cores = num_cores)
...
full_100Kg_1 <- mclapply(1:32, function (x) {
  run_sim(million_x, million_y_g, gaussian.loglik.aic.irlssgd, 100000, model_partitions[x,], 0.01)
}, mc.cores = num_cores)
...
full_100Kg_5 <- mclapply(1:32, function (x) {
  run_sim(million_x, million_y_g, gaussian.loglik.aic.irlssgd, 100000, model_partitions[x,], 0.05)
}, mc.cores = num_cores)
full_100Kg_5 <- unlist(full_100Kg_5, recursive = F)
save(full_100Kg_5, file="full_100Kg_5.Rdata")

full_100Kg_10 <- mclapply(1:32, function (x) {
  run_sim(million_x, million_y_g, gaussian.loglik.aic.irlssgd, 100000, model_partitions[x,], 0.1)
}, mc.cores = num_cores)
full_100Kg_10 <- unlist(full_100Kg_10, recursive = F)
save(full_100Kg_10, file="full_100Kg_10.Rdata")