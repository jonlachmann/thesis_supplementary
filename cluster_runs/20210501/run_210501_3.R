...
# Run using full data
run_multisim(million_x, million_y_l, logistic.loglik.bic, model_partitions, n_obs, 1, basename, dirname)

# Run using various subsample sizes
for (subs in subs_list) {
  name <- paste0(basename, subs*100)
  name <- gsub("\\.", "", name)
  run_multisim(million_x, million_y_l, logistic.loglik.bic.irlssgd, model_partitions, n_obs, subs, name, dirname)
}