...
basename <- paste0("run",dirname, "_full_10Kl")
subs_list <- c(0.2,0.1,0.05,0.01,0.0075,0.005,0.0025,0.001,0.0005)

# Run using full data
run_multisim(mill_x_g, mill_y_l10K, logistic.loglik.bic, model_partitions, n_obs, 1, basename, dirname)
run_clustersim(mill_x_g, mill_y_l10K, logistic.loglik.bic, model_partitions, n_obs, 1, basename, dirname)

# Run using various subsample sizes
for (subs in subs_list) {
  name <- paste0(basename, subs*100)
  name <- gsub("\\.", "", name)
  run_multisim(mill_x_g, mill_y_l10K, logistic.loglik.bic.irlssgd, model_partitions, n_obs, subs, name, dirname)
}